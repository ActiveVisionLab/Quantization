...
def conv3x3(in_planes, out_planes, block_size, bit, stride=1, groups=1, dilation=1):
    ...
    return DSConv2d(in_planes, out_planes, kernel_size=3, block_size=block_size, bit=bit,
...
def conv1x1(in_planes, out_planes, block_size, bit, stride=1):
    """1x1 convolution"""
    return DSConv2d(in_planes, out_planes, kernel_size=1, block_size=block_size, bit=bit,
...
class ResNet(QuantizedModule):
    ...
    def __init__(self, block, layers, bits, number_bits, num_classes=1000, zero_init_residual=False,
...
class QuantizedResNet101(ResNet):
    number_bits = 104
    def __init__(self, bits, block_size, pretrained=False, progress=False, **kwargs):
        super(QuantizedResNet101, self).__init__(Bottleneck, [3, 4, 23, 3], bits, self.number_bits, **kwargs)
        if pretrained:
...
class QuantizedResNet152(ResNet):
    number_bits = 154
    def __init__(self, bits, block_size, pretrained=False, progress=False, **kwargs):
        super(QuantizedResNet152, self).__init__(Bottleneck, [3, 8, 36, 3], bits, self.number_bits, **kwargs)
        if pretrained:
