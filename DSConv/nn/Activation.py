...
class BFPQuant(torch.autograd.Function):
    ...
    def __to_exponent_mantissa_width__(
        inp, max_log, mantissa_bitwidth, min_mantissa, max_mantissa
    ):
        shp = inp.shape
        max_log = max_log.unsqueeze(2)
        # NOTE THAT THIS IS -1 BECAUSE OF THE LEADING 1 IN 1.b0b1b2b3...*2^E
        exponent_needed = (mantissa_bitwidth - max_log - 1) * torch.ones(shp).to(
            inp.device
        )
    ...
    def forward(ctx, inp, min_e, max_e, mantissa_bit, min_m, max_m):
        ...
        quantized_act = BFPQuant.__to_exponent_mantissa_width__(
