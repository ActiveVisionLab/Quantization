...
__global__ void
forward_kernel(const torch::PackedTensorAccessor32<float, 5, torch::RestrictPtrTraits> activations,
    ...
    const int32_t w = wh % W;
    const int32_t h = (wh - w) / W;
    ...
    for (int32_t b = 0; b < blocks; b++) {
        ...
        if (c < C) {
            ...
            memcpy(&data, &activations[n][w][h][d][c], sizeof(uint32_t));
        }
...
std::vector<at::Tensor> bfpactivation3d_cuda_forward(const torch::Tensor activations,
    ...
    auto output = torch::zeros({N, W, H, D, C},
        ...
        AT_DISPATCH_FLOATING_TYPES(
            activations.scalar_type(), "bfpactivation_forward_cuda", ([&] {
                forward_kernel<scalar_t, 16><<<grid, block>>>(
                    activations.packed_accessor32<float, 5, torch::RestrictPtrTraits>(),
                    output.packed_accessor32<float, 5, torch::RestrictPtrTraits>(), m_bits,
                    trunc_num, round_num);
            }));
